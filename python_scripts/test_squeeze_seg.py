...
def main():
  script,filename,save_iter = argv
#   left_side = np.zeros((64,263,3))
  # right_side = np.zeros((64,283,3))
  
  # left_side_rotation = np.zeros((64,263,1))
  # right_side_rotation = np.zeros((64,283,1))

  # left_side_complete = np.zeros((64,263,2))
  # right_side_complete = np.zeros((64,283,2))
  # results_dir = '/home/dewan/dewan-iview_tracking_ws/test_code/results/deep_learning_results/'
  ...
  with tf.Session() as sess:
    ...
    num_classes = 4
    num_channels = 1
    model_name = '../models/' + filename + '_' + save_iter + '.ckpt.meta'
#    model_name = '../models/' + filename +'-' + save_iter + '.meta'

    new_saver = tf.train.import_meta_graph(model_name)

    print('loaded graph')
    model_name = '../models/' + filename + '_' + save_iter + '.ckpt'
    new_saver.restore(sess,model_name)
    graph = tf.get_default_graph()
    #for op in sess.graph.get_operations():
     # if('Final_decoder' in op.name):
      #  print(op.name)
       # raw_input()
